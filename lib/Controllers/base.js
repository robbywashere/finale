            ...
            var decisionPromise = new Promise(function(resolve) {
              _.assign(context, {
                ...
                error: function(status, message, errorList, cause) {
                  ...
                  // passed an error to rethrow, otherwise build an FinaleError
                  if (_.isUndefined(message) || status instanceof errors.FinaleError) {
                    resolve(status);
                  } else {
                    resolve(new errors.FinaleError(status, message, errorList, cause));
                  }
                }
              });
            });
